...
export default function TabChat({ clase }) {
  ...
  return (
    <div className="card flex flex-col h-[calc(100vh-340px)] min-h-[400px] sm:h-[60vh]">
            ...
            return (
              <div key={m.id} className={`flex ${mio ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] sm:max-w-[75%] rounded-2xl px-3 sm:px-4 py-2 ${
                  ...
                  {!mio && (
                    <p className="text-[11px] sm:text-xs font-medium text-accent-deep mb-0.5">
                      {m.autorNombre}
                    </p>
                  )}
      ...
      {puedeEscribir ? (
        ...
        <div className="border-t border-ink-100 p-3 text-center text-sm text-ink-500">
          Inscríbete a la clase para participar.
        </div>
      )}
    </div>
  );
