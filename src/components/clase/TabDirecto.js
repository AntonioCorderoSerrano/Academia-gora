          ...
          <div className="card p-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-display text-xl">{meeting.topic}</h3>
                {meeting.start_time && (
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <a href={meeting.join_url} target="_blank" rel="noreferrer" className="btn-accent">
                <Play size={16} /> Entrar al directo
              </a>
              <button onClick={cargarGrabaciones} className="btn-outline">
  ...
  return (
    <div className="space-y-6">
      {meeting ? (
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-ink-500 mt-2">
                ID de reunión: <span className="font-mono">{meeting.id}</span>
              </p>
            </div>
            <button onClick={borrarReunion} className="btn-ghost text-red-700">
              <Trash2 size={16} />
            </button>
          </div>

          <div className="mt-4 flex flex-wrap gap-2">
            <a href={meeting.start_url} target="_blank" rel="noreferrer" className="btn-accent">
              <Play size={16} /> Iniciar como host
            </a>
            <a href={meeting.join_url} target="_blank" rel="noreferrer" className="btn-outline">
              <ExternalLink size={16} /> Enlace de alumno
            </a>
            <button onClick={cargarGrabaciones} className="btn-outline">
        ...
        <div className="card p-6">
          <h3 className="font-display text-xl mb-1">Programar clase en directo</h3>
          ...
          <form onSubmit={crearReunion} className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-ink-700">
                  Fecha y hora (opcional - si se omite, reunión instantánea)
                </label>
            ...
            <button disabled={loadingCreate} className="btn-primary">
        ...
        {items.map((r) => (
          <div key={r.id} className="p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium">{r.recording_type || 'Grabación'}</p>
              <p className="text-xs text-ink-500">
                {r.recording_start && new Date(r.recording_start).toLocaleString('es-ES')}
                {r.file_size ? ` · ${(r.file_size / 1048576).toFixed(1)} MB` : ''}
              </p>
            </div>
            <a href={r.play_url} target="_blank" rel="noreferrer" className="btn-outline text-sm">
              <Play size={14} /> Ver
            </a>
          </div>
        ))}
