...
export default function HomePage() {
  return (
    <main className="relative overflow-hidden min-h-screen">
      {/* NAV */}
      <header className="mx-auto flex max-w-6xl items-center justify-between px-4 sm:px-6 py-3 sm:py-5 gap-2">
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 pt-6 sm:pt-10 pb-16 sm:pb-24 md:pt-16 md:pb-32">
        <div className="grid md:grid-cols-12 gap-6 md:gap-12 items-end">
          <div className="md:col-span-8">
            <span className="chip bg-ink-900 text-accent-soft uppercase tracking-widest text-[9px] sm:text-[10px] md:text-xs">
              Plataforma educativa
            </span>
            <h1 className="mt-4 sm:mt-5 md:mt-6 font-display text-[2rem] xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-[1] md:leading-[0.95] tracking-tight">
              Enseñar, aprender y<br/>
              <em className="text-accent-deep not-italic font-normal italic">administrar</em>
              <span className="text-ink-400"> — </span>
              en un mismo lugar.
            </h1>
            <p className="mt-4 sm:mt-6 max-w-xl text-ink-600 text-sm sm:text-base md:text-lg leading-relaxed">
              Academia Ágora es una plataforma integral para academias: clases, alumnos, asistencia,
          ...
          <div className="md:col-span-4 md:pl-8 md:border-l border-ink-200 hidden md:block">
        ...
        <div className="mt-12 sm:mt-16 md:mt-24 grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-4 md:gap-6">
          {[
            { icon: GraduationCap, title: '4 roles', desc: 'Admin, maestro, alumno, tutor' },
            { icon: Users, title: 'Clases', desc: 'Inscripciones y chat' },
            { icon: CalendarCheck, title: 'Asistencia', desc: 'Registro y calendario' },
            { icon: Wallet, title: 'Pagos Stripe', desc: 'Suscripción + único' },
          ].map((f, i) => (
            <div key={i} className="card p-3 sm:p-5 md:p-6">
      ...
      <footer className="mx-auto max-w-6xl px-4 sm:px-6 pb-6 sm:pb-10 text-[11px] sm:text-sm text-ink-500 flex flex-col sm:flex-row justify-between gap-1 sm:gap-2">
        <span>© {new Date().getFullYear()} Academia Ágora</span>
