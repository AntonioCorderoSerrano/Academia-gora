...
export default function NuevaClasePage() {
  ...
  return (
    <div>
      ...
      <h1 className="font-display text-4xl mb-2">Nueva clase</h1>
      <p className="text-ink-600 mb-8">Completa la información. Podrás editarla luego.</p>

      <form onSubmit={onSubmit} className="card p-6 md:p-8 space-y-5 max-w-2xl">
        <div>
        ...
        <div className="flex gap-3 pt-2">
          <button disabled={loading} className="btn-accent">
            {loading ? 'Creando…' : 'Crear clase'}
          </button>
          <Link href="/dashboard/clases" className="btn-ghost">Cancelar</Link>
        </div>
      </form>
    </div>
  );
