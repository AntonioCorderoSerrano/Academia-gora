...
export async function POST(req) {
  try {
    ...
    if (!claseId || !alumnoUid) return NextResponse.json({ error: 'Faltan datos' }, { status: 400 });
    ...
    const aSnap = await adminDb.ref(`usuarios/${alumnoUid}`).once('value');
    const alumno = aSnap.val();
