...
import {
  ...
  MessageSquare, CreditCard, LogOut, Settings, MoreHorizontal, UserCircle,
} from 'lucide-react';

const NAV = [
  { href: '/dashboard', label: 'Inicio', icon: Home, roles: ['*'], priority: 1 },
  { href: '/dashboard/clases', label: 'Clases', icon: BookOpen, roles: ['*'], priority: 1 },
  { href: '/dashboard/hijos', label: 'Hijos', icon: UserCircle,
    roles: [ROLES.TUTOR], priority: 2 },
  { href: '/dashboard/calendario', label: 'Agenda', icon: Calendar, roles: ['*'], priority: 2 },
  { href: '/dashboard/mensajes', label: 'Mensajes', icon: MessageSquare, roles: ['*'], priority: 2 },
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    ...
    else if (status === SESSION_STATE.UNVERIFIED || status === SESSION_STATE.KICKED) {
      router.push('/login');
    }
  }, [status, router]);
