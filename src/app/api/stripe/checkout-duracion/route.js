...
export async function POST(req) {
  try {
    ...
    const esMensual = clase.unidadDuracion === 'meses';
    const precio = esMensual
      ? Math.round(Number(clase.precioMensual) * 100)
      : Math.round(Number(clase.precioSemanal) * 100);
    const interval = esMensual ? 'month' : 'week';
    const numeroPeriodos = Number(clase.numeroPeriodos);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          recurring: { interval },
          product_data: {
            name: `${clase.nombre} — ${numeroPeriodos} ${clase.unidadDuracion}`,
          },
          unit_amount: precio,
        },
        quantity: 1,
      ...
      subscription_data: {
        metadata: {
          uid, claseId, tipo: 'duracion', hijoId: hijoId || '',
          numeroPeriodos: String(numeroPeriodos),
          unidad: clase.unidadDuracion,
        },
      },
